...
import {
    ...
    Layers, Briefcase, CreditCard, Image as ImageIcon, Newspaper, Star, MoreVertical
} from 'lucide-react';
    ...
    return (
        <aside className={`custom-sidebar fixed left-0 top-0 h-screen z-[1000] overflow-y-auto overflow-x-hidden flex flex-col transition-all duration-400 ease-[cubic-bezier(0.4,0,0.2,1)] bg-[var(--surface-color)] text-[var(--text-dark)] border-r border-[var(--border-color)] ${isOpen ? 'w-[var(--sidebar-width)]' : 'w-[var(--sidebar-collapsed-width)]'}`}>
            ...
            <div className="py-2.5 flex-1 w-full">
                {isOpen && <div className="px-6 py-3 pb-1 text-[11px] font-bold text-[var(--text-muted)] tracking-widest uppercase">User Menu</div>}
            ...
            <div className="sidebar-footer p-4 border-t border-[var(--border-color)]">
               <div className="flex items-center gap-3 bg-[var(--primary)]/5 p-3 rounded-2xl border border-[var(--primary)]/10 overflow-hidden">
                   <div className="w-8 h-8 rounded-full bg-[var(--primary)] text-white flex items-center justify-center font-black text-xs">U</div>
                   {isOpen && <div>
                       <div className="text-[10px] font-black text-[var(--text-dark)] uppercase leading-none">User Hub</div>
                       <div className="text-[9px] font-bold text-[var(--text-muted)] italic leading-none mt-1 uppercase tracking-tighter">Premium Access</div>
                   </div>}
               </div>
            </div>
        </aside>
