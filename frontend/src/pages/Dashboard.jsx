        ...
        <div className="page-content animate-fade-in space-y-8 pb-20">
            ...
            <div className="relative overflow-hidden rounded-[2.5rem] bg-indigo-600 p-8 md:p-12 shadow-2xl shadow-indigo-200 dark:shadow-indigo-900/20">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div className="w-full">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/30 text-indigo-100 text-[10px] font-black uppercase tracking-widest mb-4 backdrop-blur-md border border-white/10">
                        ...
                        <h1 className="text-4xl md:text-5xl font-black text-white mb-4 tracking-tight leading-tight uppercase">
                            {greeting}, <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-100 to-white/70">Admin</span>
                        </h1>
                        <p className="text-indigo-100/70 font-medium text-lg leading-relaxed max-w-xl">
                            Welcome back! Here's a look at how your platform is performing today.
                        </p>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                        <div className="px-5 py-3 rounded-2xl bg-white/10 border border-white/10 backdrop-blur-xl flex items-center gap-3 text-white">
                            ...
                            <span className="text-xs font-black uppercase tracking-widest leading-none mt-0.5">System Online</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    data.summary.map((stat, i) => (
                        <div key={i} className="card group hover:translate-y-[-4px] transition-all duration-300 border border-[var(--border-color)] shadow-sm hover:shadow-xl hover:shadow-indigo-500/10 bg-[var(--surface-color)] p-6 rounded-[2rem] relative overflow-hidden">
                            <div className="flex items-center justify-between mb-4 relative z-10">
                                ...
                                <div className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 text-[10px] font-black">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="card p-8 bg-[var(--surface-color)] border border-[var(--border-color)] rounded-[2.5rem] shadow-sm overflow-hidden h-full">
                        <div className="flex items-center justify-between mb-10">
                            <div>
                                <h3 className="text-lg font-black text-[var(--text-dark)] uppercase tracking-tight leading-none mb-1">Leads Growth</h3>
                                <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest">Weekly ads performance trend</p>
                            </div>
                ...
                <div className="space-y-6 flex flex-col h-full">
                    <div className="card flex-1 p-8 bg-[var(--surface-color)] border border-[var(--border-color)] rounded-[2.5rem] shadow-sm relative overflow-hidden">
                        ...
                        <div className="flex items-center justify-between mb-8 relative z-10">
                            <h3 className="text-sm font-black text-[var(--text-dark)] uppercase tracking-tight">Top Vendors</h3>
                            <Link to="/analytics" className="text-[9px] font-black text-indigo-600 uppercase tracking-widest hover:translate-x-1 transition-all flex items-center gap-1">View All <ArrowRight size={10} /></Link>
                        </div>
                        ...
                        <div className="space-y-8 relative z-10">
                            {data.topVendors.length > 0 ? (
                                    ...
                                    <div className="space-y-4">
                                        {data.topVendors.slice(0, 3).map((vendor, i) => (
                                            <div key={i} className="flex items-center gap-3 p-3 rounded-2xl bg-[var(--bg-color)]/30 border border-[var(--border-color)] group hover:border-indigo-500 transition-all">
                                                <div className="w-8 h-8 rounded-lg bg-indigo-500/10 text-indigo-600 flex items-center justify-center font-black text-[10px]">#{i+1}</div>
                                                <div className="flex-1">
                                                    <div className="text-[11px] font-black text-[var(--text-dark)] uppercase tracking-tight leading-none mb-1">{vendor.name}</div>
                                                    <div className="text-[8px] font-bold text-[var(--text-muted)] leading-none">{vendor.leads_uploaded} Leads</div>
                                                </div>
                                ...
                                <div className="text-center py-10 text-[var(--text-muted)] text-[10px] font-bold uppercase tracking-widest">No active vendors found</div>
                            )}
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div key={i} className="flex gap-4 group">
                                ...
                                <div className="flex-1">
                                    <div className="flex justify-between items-baseline mb-1">
                                        ...
                                        <span className="text-[8px] font-bold text-[var(--text-muted)]">{new Date(activity.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                                    </div>
                                    <p className="text-[10px] font-medium text-[var(--text-muted)]">{activity.detail}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                        { title: 'Add Lead', path: '/leads/create', icon: Plus, color: 'indigo' },
                        { title: 'Vendors', path: '/vendors', icon: ShieldCheck, color: 'emerald' },
                        { title: 'Analytics', path: '/analytics', icon: Activity, color: 'indigo' },
                        { title: 'Plans', path: '/subscriptions', icon: Layers, color: 'rose' }
                    ].map((action, i) => (
                        <Link
                            key={i}
                            to={action.path}
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between">
                                <div className="font-black text-xs text-[var(--text-dark)] uppercase tracking-widest">{action.title}</div>
