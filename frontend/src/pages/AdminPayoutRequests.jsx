...
import {
    ...
    MessageSquare, CreditCard, AlertCircle, FileText, Send
} from 'lucide-react';
...
const AdminPayoutRequests = () => {
    ...
    const fetchPayouts = useCallback(async () => {
        ...
        try {
            ...
            setMessage({ type: 'error', text: 'Protocol synchronization error while loading payouts.' });
        } finally {
            setLoading(false);
        }
    }, [filterStatus]);
    ...
    const handleActionClick = (payout, status) => {
        ...
        setModalAction({
            ...
            remarks: status === 'COMPLETED'
                ? 'Payout processed successfully via designated channel.'
                : 'Payout request declined after audit review.',
            payment_ref: ''
        });
        setIsModalOpen(true);
    };
    ...
    const handleProcessPayout = async () => {
        if (!modalAction.remarks) {
            alert('Remarks are required for audit trail.');
            return;
        }
        ...
        try {
            ...
            if (data.success) {
                setMessage({
                    type: 'success',
                    text: `Payout ${selectedPayout.id.slice(0, 8)} updated to ${modalAction.status}.`
                });
            ...
            setMessage({ type: 'error', text: 'Modification rejected by payout service.' });
        } finally {
    ...
    const filteredPayouts = payouts.filter((payout) =>
        payout.vendor_name?.toLowerCase().includes(search.toLowerCase())
        || payout.vendor_phone?.includes(search)
    );
        ...
        <div className="animate-fade-in space-y-8 pb-32">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-xs font-bold text-[var(--text-muted)] italic">Manage and authorize vendor payout requests.</p>
                </div>
                <div className="flex items-center gap-4">
                    <div className="bg-[var(--surface-color)] p-4 rounded-3xl border border-[var(--border-color)] flex items-center gap-4 shadow-sm group hover:scale-105 transition-all">
                        ...
                        <div>
                            ...
                            <h4 className="text-xl font-black text-[var(--text-dark)] tracking-tighter">
                                {payouts.filter((payout) => payout.status === 'REQUESTED').length} <span className="text-[9px]">PENDING</span>
                            </h4>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-3 w-full md:w-auto">
                    {['', 'REQUESTED', 'COMPLETED', 'FAILED'].map((status) => (
                        <button
                            key={status}
                        ...
                        >
                            {status === 'REQUESTED' ? 'PENDING' : (status || 'ALL_NODES')}
                        </button>
                    ))}
                </div>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-[var(--bg-color)]/50 border-b border-[var(--border-color)]">
                                ...
                                <th className="py-8 text-left text-[9px] uppercase font-black text-[var(--text-muted)] tracking-[0.2em]">Amount</th>
                        ...
                        <tbody className="divide-y divide-[var(--border-color)]/30">
                            {loading ? (
                                <tr>
                                    <td colSpan="5" className="py-40 text-center text-[10px] font-black text-indigo-500 uppercase tracking-[0.5em] animate-pulse">Scanning financial sub-matrix...</td>
                                </tr>
                            ) : filteredPayouts.length === 0 ? (
                                <tr>
                                    <td colSpan="5" className="py-40 text-center text-xs font-bold text-[var(--text-muted)] italic leading-relaxed">No payout transmissions detected in this grid sector.</td>
                                </tr>
                            ) : (
                                    ...
                                    <tr key={payout.id} className="group hover:bg-white/[0.01] transition-colors">
                                        ...
                                        <td>
                                            <div className="flex items-baseline gap-1 text-2xl font-black text-[var(--text-dark)] tracking-tighter">
                                                <span className="text-xs text-indigo-500 font-bold">Rs</span>
                                        ...
                                        <td>
                                            <span className={`px-4 py-1.5 rounded-full border text-[9px] font-black uppercase tracking-widest inline-flex items-center gap-2 ${getStatusStyle(payout.status)}`}>
                                                <div className={`w-1.5 h-1.5 rounded-full bg-current ${payout.status === 'REQUESTED' ? 'animate-pulse' : ''}`}></div>
                                                {payout.status}
                                            </span>
                                        </td>
                                            ...
                                            {payout.status === 'REQUESTED' ? (
                                                ...
                                                <button
                                                    onClick={() => {
                                                        setSelectedPayout(payout);
                                                        setIsModalOpen(true);
                                                        setModalAction({ status: 'VIEW', remarks: payout.remarks, payment_ref: payout.payment_reference });
                                                    }}
                            ...
                            <div className="space-y-8">
                                <div className="p-6 bg-[var(--bg-color)] rounded-3xl border border-[var(--border-color)] flex items-center justify-between shadow-inner">
                                    <div>
                                        ...
                                        <h4 className="text-3xl font-black text-[var(--text-dark)] tracking-tighter">Rs{parseFloat(selectedPayout?.amount || 0).toLocaleString()}</h4>
                                    </div>
