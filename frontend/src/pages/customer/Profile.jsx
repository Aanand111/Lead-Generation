...
const UserProfile = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                setMessage({ type: 'success', text: 'Identity protocol synchronized.' });
            }
        } catch (err) {
            setMessage({ type: 'error', text: 'Update rejected by central core.' });
        } finally {
            setLoading(false);
        ...
        <div className="page-content animate-fade-in text-[var(--text-dark)] pb-20">
            <div className="pageHeader">
                <div className="pageHeaderTitle">
                    ...
                    <p>Manage your neural network profile and security synchronization</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                <div className="lg:col-span-1 space-y-10">
                    <div className="card p-12 bg-[var(--surface-color)] border border-[var(--border-color)] text-center relative overflow-hidden rounded-[3rem] shadow-xl group">
                        ...
                        <p className="text-[11px] font-black text-indigo-500 uppercase tracking-widest italic mb-8 opacity-70 leading-none">Primary Neural Node</p>
                        ...
                        <div className="space-y-4 pt-10 border-t border-[var(--border-color)]">
                             <div className="flex items-center justify-between p-4 rounded-2xl bg-[var(--bg-color)]/50 border border-[var(--border-color)] group hover:border-indigo-500/30 transition-all cursor-default relative overflow-hidden">
                                <div className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest leading-none">Wallet Status</div>
                                <div className="text-sm font-black text-emerald-500 tracking-widest tabular-nums uppercase leading-none">ACTIVE SPECTRUM</div>
                             </div>
                             <div className="flex items-center justify-between p-4 rounded-2xl bg-[var(--bg-color)]/50 border border-[var(--border-color)] group hover:border-indigo-500/30 transition-all cursor-default relative overflow-hidden">
                                <div className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest leading-none">Yield Multiplier</div>
                                <div className="text-sm font-black text-indigo-500 tracking-widest tabular-nums uppercase leading-none">X1.4 CORE</div>
                             </div>
                        </div>
                    </div>
                    ...
                    <div className="card p-10 shadow-lg border border-[var(--border-color)] bg-[var(--surface-color)] group hover:border-indigo-500/20 transition-all flex items-center justify-between rounded-[2.5rem] relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                <h4 className="text-xl font-black italic tracking-tighter uppercase text-[var(--text-dark)] mb-1 leading-none">Security Core</h4>
                                <p className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest italic opacity-70 leading-none mt-2">Synchronize Credentials</p>
                            </div>
                        </div>
                    ...
                    <div className="card shadow-2xl border border-[var(--border-color)] bg-[var(--surface-color)] rounded-[3rem] overflow-hidden">
                        <div className="p-10 border-b border-[var(--border-color)] bg-[var(--bg-color)]/30 flex items-center justify-between">
                             <div className="flex items-center gap-4">
                                <h3 className="text-2xl font-black italic tracking-tighter uppercase text-[var(--text-dark)]">Synchronization Center</h3>
                                <div className="h-6 w-px bg-[var(--border-color)]"></div>
                                <ShieldCheck size={24} className="text-emerald-500" />
                            </div>
                            <div className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] italic">Encryption Level: AES-256</div>
                        </div>
                        
                        <form onSubmit={handleSubmit} className="p-10 lg:p-14 space-y-12">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div className="space-y-4">
                                    <label className="text-[11px] font-black italic text-[var(--text-dark)] uppercase tracking-[0.2em] ml-2 flex items-center gap-2">
                                        <User size={14} className="text-indigo-500" /> Identity Label
                                    </label>
                                    <input 
                                ...
                                <div className="space-y-4">
                                    <label className="text-[11px] font-black italic text-[var(--text-dark)] uppercase tracking-[0.2em] ml-2 flex items-center gap-2">
                                        <Smartphone size={14} className="text-indigo-500" /> Neural Contact
                                    </label>
                                    <input 
                                    ...
                                    />
                                    <div className="text-[9px] font-black text-rose-500 uppercase tracking-widest mt-1 ml-2 italic">Phone number is biologically locked</div>
                                </div>
                                <div className="space-y-4">
                                    <label className="text-[11px] font-black italic text-[var(--text-dark)] uppercase tracking-[0.2em] ml-2 flex items-center gap-2">
                                        <Mail size={14} className="text-indigo-500" /> Communication Index
                                    </label>
                                    <input 
                                ...
                                <div className="space-y-4">
                                    <label className="text-[11px] font-black italic text-[var(--text-dark)] uppercase tracking-[0.2em] ml-2 flex items-center gap-2">
                                        <Globe size={14} className="text-indigo-500" /> Geolocation Hub
                                    </label>
                            ...
                            <div className="space-y-4">
                                <label className="text-[11px] font-black italic text-[var(--text-dark)] uppercase tracking-[0.2em] ml-2 flex items-center gap-2">
                                    <MapPin size={14} className="text-indigo-500" /> Postal Protocol (PIN)
                                </label>
                                <input 
                                    ...
                                    placeholder="Encryption Zone Code"
                                />
                            </div>
