...
const UserAvailableLeads = () => {
    ...
    const fetchLeads = async (isQuiet = false) => {
        ...
        try {
            ...
            setError("Unable to sync with the lead spectrum.");
        } finally {
    ...
    const handlePurchase = async (leadId, cost = 10) => {
        if (userCredits < cost) {
            setMessage({ type: 'error', text: 'INSUFFICIENT CREDENTIALS. ACQUISITION PROTOCOL REJECTED.' });
        ...
        try {
            ...
            if (data.success) {
                setMessage({ type: 'success', text: 'LEAD ENCRYPTION KEY ACQUIRED. TRANSFER COMPLETE.' });
                fetchLeads(true); // Refresh credits and list quietly
            } else {
                setMessage({ type: 'error', text: data.message || "ACQUISITION REJECTED." });
            }
        } catch (err) {
            const errorMsg = err.response?.data?.message || 'FINANCIAL TRANSACTION PROTOCOL FAILURE.';
    ...
    const getTimeAgo = (dateString) => {
        ...
        if (diffInHours < 1) return 'Just synchronization';
                ...
                <div className="pageHeader mb-12">
                    <div className="pageHeaderTitle">
                        <div className="flex items-center gap-3 mb-4">
                            <div className="px-3 py-1 bg-black text-white text-[9px] font-black uppercase tracking-[0.2em] rounded-full animate-pulse shadow-lg shadow-indigo-500/20">
                                Live Spectrum
                            </div>
                        ...
                        <h2 className="text-5xl font-black italic tracking-tighter uppercase text-slate-900 leading-tight">
                            Lead <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-blue-500">Extraction</span>
                        </h2>
                        <p className="text-slate-400 font-bold text-sm tracking-wide mt-2 max-w-xl italic">
                            Scanning neural networks for high-conversion industry nodes. Acquire encrypted data keys for immediate integration.
                        </p>
                    </div>
                    ...
                    <div className="pageHeaderActions flex items-center gap-6">
                        <div className="flex items-center gap-3 bg-[var(--surface-color)]/30 backdrop-blur-md p-2 pl-5 rounded-3xl border border-[var(--border-color)]">
                             <div className="flex flex-col">
                                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none">Intelligence Feed</span>
                                <span className={`text-[10px] font-black uppercase tracking-widest ${autoRefresh ? 'text-emerald-500' : 'text-slate-400'}`}>
                                    {autoRefresh ? 'LIVE STREAM' : 'STATIC NODE'}
                                </span>
                             </div>
                             <button 
                        ...
                        <div className="glass-morphism p-1 rounded-3xl border border-[var(--border-color)] shadow-xl overflow-hidden flex items-center">
                            <div className="px-8 py-4 bg-[var(--surface-color)] rounded-2xl flex items-center gap-4 transition-transform hover:scale-[1.02] cursor-default border border-[var(--border-color)] shadow-sm">
                                <div className="space-y-1">
                                    <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Wallet Token</div>
                                    <div className="flex items-center gap-2">
                ...
                <div className="mb-16 p-2 rounded-[2.5rem] bg-[var(--surface-color)] border border-[var(--surface-color)] shadow-2xl shadow-slate-200/50 flex flex-col md:flex-row items-center gap-2 transition-all focus-within:border-indigo-500/30">
                    <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-3 gap-2">
                        <div className="relative group">
                            ...
                            <input 
                                type="text" placeholder="Scan City..." 
                        ...
                        <div className="relative group">
                            ...
                            <input 
                                type="text" placeholder="Industry Domain..." 
                    ...
                    <button onClick={fetchLeads} className="w-full md:w-auto px-10 py-5 bg-[var(--surface-color)] text-indigo-600 rounded-3xl font-black uppercase tracking-[0.2em] text-[11px] shadow-xl hover:bg-[var(--surface-color)] transition-all active:scale-95 flex items-center justify-center gap-3">
                        <Search size={18} /> Start Extraction
                    </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
                    {loading ? (
                            ...
                            <div key={lead.id} className="group relative">
                                ...
                                <div className="bg-[var(--surface-color)] border border-[var(--surface-color)] p-10 rounded-[3.5rem] shadow-sm group-hover:shadow-2xl group-hover:border-indigo-500/20 transition-all duration-500 flex flex-col h-full overflow-hidden relative">
                                    ...
                                    <div className="flex items-center justify-between mb-8">
                                        ...
                                        <div className="flex items-center gap-2 text-[10px] font-black text-slate-300 uppercase tracking-widest italic leading-none">
                                            <Clock size={12} />
                                    ...
                                    <div className="flex items-start gap-6 mb-8">
                                        ...
                                        <div className="pt-2">
                                            <div className="text-[9px] font-[var(--surface-color)] text-indigo-500 uppercase tracking-[0.2em] mb-1 italic">Industrial Sector</div>
                                            <h4 className="text-[var(--surface-color)] font-[var(--surface-color)] text-2xl italic uppercase text-slate-700 leading-tight">
                                                {lead.category_name || 'Neural Extraction'}
                                            </h4>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-6 flex-1">
                                        ...
                                        <div className="p-5 bg-[var(--surface-color)] rounded-3xl space-y-2 border border-slate-50">
                                            <p className="text-[11px] font-bold text-slate-400 font-mono tracking-wide break-all opacity-60 italic">
                                                {lead.contact_hint_1 || 'xxxxxxxx@node.ext'}
                                            </p>
                                            <p className="text-[11px] font-bold text-slate-400 font-mono tracking-widest opacity-60 italic">
                                                {lead.contact_hint_2 || '+91 xxxx xxx 867'}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-10 pt-8 border-t border-slate-50 flex items-center justify-between gap-6">
                                        <div className="space-y-1">
                                            <div className="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none">Extraction Cost</div>
                                            <div className="text-lg font-black text-slate-800 italic">{lead.credit_cost || 10} <span className="text-[10px] uppercase text-indigo-500 not-italic">CR</span></div>
                                        </div>
                                        <button 
                        ...
                        <div className="col-span-full py-40 text-center glass-morphism rounded-[3rem] border border-slate-100 flex flex-col items-center gap-8 opacity-20">
                            ...
                            <p className="font-black uppercase tracking-[0.4em] text-xs italic">Spectral signal lost. Re-targeting recommended.</p>
                        </div>
                    )}
                </div>

                {/* Footer Top-up Banner */}
                <div className="mt-20 p-12 rounded-[4rem] bg-indigo-600 text-white flex flex-col md:flex-row items-center justify-between gap-10 relative overflow-hidden group shadow-2xl shadow-indigo-500/20">
                    ...
                    <div className="relative z-10 max-w-xl text-center md:text-left">
                        <h3 className="text-3xl font-black italic tracking-tighter uppercase mb-4 leading-none">Protocol Interrupted?</h3>
                        <p className="text-sm font-medium text-white/70 leading-relaxed uppercase tracking-widest">
                            Your wallet token supply is low. Synchronize with the extraction network to maintain lead acquisition speed.
                        </p>
                    </div>
                    <button className="relative z-10 bg-white text-indigo-600 px-10 py-6 rounded-[2.5rem] font-black uppercase tracking-[0.2em] text-[11px] shadow-2xl hover:scale-105 active:scale-95 transition-all duration-300">
                        Synchronize Wallet
                    </button>
                </div>
            </div>
