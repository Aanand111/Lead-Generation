...
const SubVendorEdit = () => {
    ...
    useEffect(() => {
        const fetchSubVendor = async () => {
            setLoading(true);
            try {
                // The API for subvendors expects ID in query (based on previous files)
                const { data } = await api.get(`/admin/subvendors?id=${id}`);
                if (data.success && data.data) {
                    ...
                    setFormData({
                        ...
                        referred_by_vendor_id: sub.referred_by_vendor_id || '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Using the query param ID as seen in the original file
            const { data } = await api.put(`/admin/subvendors?id=${id}`, payload);
            if (data.success) {
                setMessage({ type: 'success', text: 'Agent node synchronized successfully.' });
                setTimeout(() => navigate('/sub-vendors'), 1500);
            } else {
                setMessage({ type: 'error', text: data.message || 'Error updating agent' });
            }
        } catch (error) {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.4em] animate-pulse">Syncing Agent...</span>
            </div>
        );
    }
            ...
            <div className="pageHeader">
                <div className="pageHeaderTitle">
                    ...
                    <h2 className="flex items-center gap-3 italic font-black uppercase tracking-tight">
                        Agent Node Refinement
                        <Shield size={24} className="text-indigo-600 animate-pulse" />
                    </h2>
                    <p>Reconfigure independent agent parameters and service protocols for Node ID: {id}</p>
                </div>
            </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-10">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            ...
                            <div className="space-y-8">
                                <div className="flex items-center gap-4 mb-2">
                                    ...
                                    <div>
                                        <h3 className="text-[10px] font-black uppercase tracking-[.4em] text-[var(--text-dark)]">Agent Identity Matrix</h3>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest ml-1 block">Legal Designation *</label>
                                    <div className="relative group">
                                        ...
                                        <input type="text" name="name" value={formData.name} onChange={handleChange} required className="w-full p-5 pl-12 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-2xl font-black text-sm uppercase tracking-tight text-[var(--text-dark)] focus:bg-white focus:border-indigo-600 transition-all shadow-inner outline-none" placeholder="AGENT_IDENTITY" />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group">
                                        ...
                                        <input type="email" name="email" value={formData.email} onChange={handleChange} required className="w-full p-5 pl-12 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-2xl font-black text-sm text-[var(--text-dark)] focus:bg-white focus:border-indigo-600 transition-all shadow-inner outline-none" placeholder="AGENT_SIGNAL@ENTITY.SYS" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-8">
                                <div className="flex items-center gap-4 mb-2">
                                    ...
                                    <div>
                                        <h3 className="text-[10px] font-black uppercase tracking-[.4em] text-[var(--text-dark)]">Hierarchy Context</h3>
                                        <p className="text-[9px] text-[var(--text-muted)] font-black uppercase tracking-widest italic mt-1 leading-none opacity-60">Network routing and parent node definitions</p>
                                    </div>
                                </div>

                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest ml-1 block">Referral ID Tag</label>
                                        <div className="relative group">
                                            <BadgeInfo size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--text-muted)] opacity-50" />
                                            <input type="text" name="referral_code" value={formData.referral_code} onChange={handleChange} className="w-full p-5 pl-12 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-2xl font-black text-sm uppercase tracking-tight text-[var(--text-dark)] focus:bg-white focus:border-indigo-600 transition-all shadow-inner outline-none" placeholder="AGENT_REF_TAG" />
                                        </div>
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest ml-1 block">Parent Node ID (Vendor)</label>
                                        <div className="relative group">
                                            ...
                                            <select 
                                                ...
                                                className="w-full p-5 pl-12 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-2xl font-black text-sm uppercase tracking-tight text-[var(--text-dark)] focus:bg-white focus:border-indigo-600 transition-all shadow-inner outline-none cursor-pointer"
                                                required
                                            >
                                                <option value="">SELECT PARENT VENDOR</option>
                                                {vendors.map(v => (
                                                    <option key={v.id} value={v.id}>{v.name.toUpperCase()} ({v.phone})</option>
                                                ))}
                                            </select>
                                        </div>
                                        {loadingVendors && <div className="text-[9px] animate-pulse text-indigo-500 ml-1 uppercase font-black tracking-widest">Hydrating network nodes...</div>}
                                    </div>
                                </div>
                                ...
                                <div className="bg-emerald-600/[0.03] p-6 rounded-[2rem] border border-emerald-600/5 mt-4">
                                    <div className="flex items-center gap-3 text-emerald-600 mb-2">
                                        <Zap size={16} />
                                        <h4 className="text-[10px] font-black uppercase tracking-widest">Agent Autonomy</h4>
                                    </div>
                                    <p className="text-[10px] text-[var(--text-muted)] font-black uppercase tracking-widest leading-relaxed italic opacity-70">
                                        Syncing this agent node will finalize their independent commission protocols and service access spectrum.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="pt-10 border-t border-[var(--border-color)] flex flex-col md:flex-row gap-6">
                            <button type="submit" className="flex-1 py-6 bg-indigo-600 hover:bg-indigo-700 text-white font-black uppercase text-[11px] tracking-[0.4em] rounded-[2.5rem] shadow-2xl shadow-indigo-600/20 flex items-center justify-center gap-4 transition-all active:scale-95 disabled:opacity-50 border-none cursor-pointer" disabled={saving}>
                                ...
                                {saving ? 'TRANSMITTING AGENT DATA...' : 'COMMIT NODE SYNCHRONIZATION'}
                            </button>
