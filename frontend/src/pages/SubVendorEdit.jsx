...
const SubVendorEdit = () => {
    ...
    useEffect(() => {
        const fetchSubVendor = async () => {
            ...
            try {
                ...
                setMessage({ type: 'error', text: 'Failed to retrieve sub-vendor profile.' });
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                setMessage({ type: 'success', text: 'Sub-vendor profile updated successfully.' });
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest animate-pulse">Loading Sub-Vendor Profile...</span>
            </div>
        );
    }
             ...
             <div className="pageHeader">
                <div className="pageHeaderTitle">
                    <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)] hover:text-indigo-600 transition-colors mb-4 bg-transparent border-none cursor-pointer p-0">
                        <ArrowLeft size={14} /> Back to Sub-Vendors
                    </button>
                    <h2 className="flex items-center gap-3 font-black uppercase tracking-tight">
                        Edit Sub-Vendor Profile
                        <Shield size={24} className="text-indigo-600" />
                    </h2>
                    <p>Update sub-vendor information and parent vendor assignments</p>
                </div>
            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            ...
                            <div className="space-y-8">
                                <div className="flex items-center gap-4 mb-2">
                                    <div className="p-3 rounded-2xl bg-indigo-600/10 text-indigo-600">
                                     ...
                                     <div>
                                        <h3 className="text-[10px] font-black uppercase tracking-widest text-[var(--text-dark)]">Sub-Vendor Details</h3>
                                        <p className="text-[9px] text-[var(--text-muted)] font-black uppercase tracking-widest mt-1 leading-none opacity-60">Personal and contact information</p>
                                    </div>
                                </div>

                                 <div className="space-y-2">
                                    <label className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest ml-1 block">Full Name *</label>
                                    <div className="relative group">
                                        ...
                                        <input type="text" name="name" value={formData.name} onChange={handleChange} required className="w-full p-5 pl-12 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-2xl font-black text-sm uppercase tracking-tight text-[var(--text-dark)] focus:bg-white focus:border-indigo-600 transition-all shadow-inner outline-none" placeholder="FULL NAME" />
                                    </div>
                                </div>
                            ...
                            <div className="space-y-8">
                                <div className="flex items-center gap-4 mb-2">
                                    <div className="p-3 rounded-2xl bg-indigo-600/10 text-indigo-600">
                                     ...
                                     <div>
                                        <h3 className="text-[10px] font-black uppercase tracking-widest text-[var(--text-dark)]">Network Hierarchy</h3>
                                        <p className="text-[9px] text-[var(--text-muted)] font-black uppercase tracking-widest mt-1 leading-none opacity-60">Parent vendor and referral assignments</p>
                                    </div>
                                </div>

                                <div className="grid grid-cols-2 gap-6">
                                     <div className="space-y-2">
                                        <label className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest ml-1 block">Referral Code</label>
                                        <div className="relative group">
                                     ...
                                     <div className="space-y-2">
                                        <label className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest ml-1 block">Parent Vendor</label>
                                        <div className="relative group">
                                         ...
                                         {loadingVendors && <div className="text-[9px] animate-pulse text-indigo-500 ml-1 uppercase font-black tracking-widest">Loading vendors...</div>}
                                    </div>
                                </div>
                                 ...
                                 <div className="bg-emerald-600/[0.03] p-6 rounded-[2rem] border border-emerald-600/5 mt-4">
                                    <div className="flex items-center gap-3 text-emerald-600 mb-2">
                                        <Zap size={16} />
                                        <h4 className="text-[10px] font-black uppercase tracking-widest">Account Settings</h4>
                                    </div>
                                    <p className="text-[10px] text-[var(--text-muted)] font-black uppercase tracking-widest leading-relaxed opacity-70">
                                        Updating this profile will reflect across the sub-vendor dashboard and associated commission reports.
                                    </p>
                                </div>
                            </div>
                        </div>
                         ...
                         <div className="pt-10 border-t border-[var(--border-color)] flex flex-col md:flex-row gap-6">
                            <button type="submit" className="flex-1 py-6 bg-indigo-600 hover:bg-indigo-700 text-white font-black uppercase text-[11px] tracking-[0.4em] rounded-[2.5rem] shadow-2xl shadow-indigo-600/20 flex items-center justify-center gap-4 transition-all active:scale-95 disabled:opacity-50 border-none cursor-pointer" disabled={saving}>
                                ...
                                {saving ? 'UPDATING...' : 'SAVE SUB-VENDOR DETAILS'}
                            </button>
