...
const SubscriptionPlanEdit = () => {
    ...
    useEffect(() => {
        const fetchPlan = async () => {
            try {
                ...
                if (res.data.success) {
                    const plan = res.data.data.find(p => String(p.id) === String(id));
                    if (plan) {
                        setFormData({
                            name: plan.name,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await api.put(`/admin/subscription-plans/${id}`, formData);
            if (res.data.success) {
                navigate('/subscriptions/plan');
            } else {
                setError(res.data.message || 'Failed to update plan');
            }
        } catch (err) {
            ...
            <div className="pageHeader">
                <div className="pageHeaderTitle">
                    <h2 className="flex items-center gap-3">
                        Modify Plan Protocol
                        <Sparkles className="text-amber-500 animate-pulse" size={24} />
                    </h2>
                    <p>Update configuration tier #{id} parameters</p>
                </div>
