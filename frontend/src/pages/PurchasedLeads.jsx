        ...
        try {
            ...
            if (data.success && data.data && data.data.length > 0) {
                const mappedData = data.data.map((lead, index) => ({
                    ...
                    leadId: lead.lead_id ? `INS-L-${index + 1000}` : 'VOID_SIG',
                    packageName: lead.package_name || 'LEGACY_VOID',
                    leadName: lead.lead_name || 'UNSPECIFIED',
        ...
        <div className="page-content animate-fade-in text-[var(--text-dark)] pb-20">
            ...
            <div className="pageHeader">
                <div className="pageHeaderTitle">
                    ...
                    <p>Monitoring global lead distributions and real-time inventory for verified customer nodes</p>
                </div>
                <div className="pageHeaderActions">
                    ...
                    <button className="btn bg-indigo-600 hover:bg-indigo-700 text-white shadow-xl shadow-indigo-500/20 flex items-center gap-3 font-black uppercase text-[10px] tracking-widest px-8 py-4 rounded-2xl transition-all active:scale-95">
                        <Plus size={18} /> Manual Acquisition
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-10">
                <div className="p-8 rounded-[2.5rem] bg-indigo-600 text-white shadow-2xl shadow-indigo-200 relative overflow-hidden group">
                    <div className="relative z-10">
                         <div className="text-[10px] font-black uppercase tracking-[0.2em] opacity-80 mb-2 italic">Active Distributions</div>
                         <div className="text-4xl font-black">{leads.length} Nodes</div>
                ...
                <div className="p-8 rounded-[3rem] bg-emerald-600 text-white shadow-2xl shadow-emerald-500/20 relative overflow-hidden group">
                    <div className="relative z-10 h-full flex flex-col justify-between">
                         <div>
                            <div className="text-[10px] font-black uppercase tracking-[0.2em] opacity-80 mb-2 italic">Remaining Flux</div>
                            <div className="text-4xl font-black">
                                {stats.total_remaining_leads || 0} <span className="text-xl font-normal opacity-60">Signal</span>
                            </div>
                         </div>
                         <div className="mt-8 flex items-center gap-2 text-[10px] uppercase font-black tracking-widest bg-black/10 px-4 py-2 rounded-2xl w-fit backdrop-blur-sm border border-white/10">
                             <Activity size={14} className="text-emerald-200" /> Buffer Capacity
                         </div>
                    </div>
                ...
                <div className="p-8 rounded-[3rem] bg-[var(--surface-color)] text-[var(--text-dark)] shadow-2xl shadow-indigo-900/10 relative overflow-hidden group border border-[var(--border-color)]">
                    <div className="relative z-10">
                         <div className="text-[11px] font-black uppercase tracking-[0.3em] text-indigo-400 mb-3 ml-1">Acquisition Strength</div>
                         <div className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[var(--text-dark)] to-indigo-400">
                             {stats.leads} <span className="text-2xl font-black italic">Units</span>
                         </div>
            ...
            <div className="card shadow-2xl rounded-[3rem] border border-[var(--border-color)] overflow-hidden bg-[var(--surface-color)] mt-10">
                <div className="p-8 border-b border-[var(--border-color)] flex flex-col md:flex-row justify-between items-center gap-6 bg-[var(--bg-color)]/30">
                    <div className="flex items-center gap-6 w-full md:w-auto">
                        ...
                        <div className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest opacity-60 hidden lg:block italic">Trace global acquisition flow protocols</div>
                    </div>
                    ...
                    <div className="relative w-full md:w-96 group">
                        ...
                        <input
                            type="text"
                            className="w-full bg-[var(--surface-color)] border border-[var(--border-color)] rounded-2xl pl-12 pr-6 py-4 text-xs font-black uppercase tracking-tight shadow-inner focus:border-indigo-500 focus:ring-8 focus:ring-indigo-600/5 outline-none transition-all placeholder:text-[var(--text-muted)]/40"
                            placeholder="TRACE NODE BY IDENTITY OR LABEL..."
                            value={searchTerm}
                    ...
                    <table className="table table-hover align-middle">
                        <thead className="bg-[#f9fafb]">
                            <tr>
                                ...
                                <th className="text-[10px] uppercase font-black text-[var(--text-muted)] tracking-widest">Customer Entity</th>
                                <th className="text-[10px] uppercase font-black text-[var(--text-muted)] tracking-widest">Lead Node Label</th>
                                <th className="text-[10px] uppercase font-black text-[var(--text-muted)] tracking-widest">Package Architecture</th>
                                <th className="text-center text-[10px] uppercase font-black text-[var(--text-muted)] tracking-widest">Flux Density</th>
                                <th className="text-center text-[10px] uppercase font-black text-[var(--text-muted)] tracking-widest">Asset Value</th>
                                <th className="text-[10px] uppercase font-black text-[var(--text-muted)] tracking-widest">Temporal Window</th>
                                <th className="text-right text-[10px] uppercase font-black text-[var(--text-muted)] tracking-widest px-10 italic">Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            {loading ? (
                                <tr><td colSpan="8" className="text-center py-40">
                                    <div className="flex flex-col items-center gap-6">
                                        ...
                                        <span className="text-[11px] font-black uppercase tracking-[0.4em] animate-pulse text-indigo-600/60">Intercepting Purchase Signals...</span>
                                    </div>
                                </td></tr>
                                ...
                                <tr><td colSpan="8" className="text-center py-40 text-[var(--text-muted)]">
                                     <div className="flex flex-col items-center gap-5 opacity-20">
                                         ...
                                         <p className="font-black uppercase tracking-[0.3em] text-[10px]">No acquisition nodes found in current spectrum</p>
                                     </div>
                                </td></tr>
                                ...
                                <tr key={lead.id} className="transition-all hover:bg-indigo-600/[0.02] group border-b border-[var(--border-color)] last:border-0 text-xs text-[var(--text-dark)]">
                                    <td className="text-center font-black text-[var(--text-muted)]/40 font-mono text-[10px] italic">#{lead.id.toString().padStart(3, '0')}</td>
                                    <td>
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest opacity-40 mt-0.5 italic">Verified Entity</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td className="text-center">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <div className="text-[8px] font-black uppercase text-gray-400 mt-1 italic tracking-widest">Flux Density</div>
                                        </div>
                                    </td>
                                    <td className="text-center">
                                    ...
                                    <td>
                                        <div className="space-y-1.5">
                                            ...
                                            <div className="flex items-center gap-2 text-[10px] font-black text-orange-500 italic opacity-60">
                                    ...
                                    <td className="text-right px-10">
                                        <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${
                                            ...
                                            {parseInt(lead.remainingLeads) > 0 ? 'NODE_ACTIVE' : 'DEPLETED'}
                                        </span>
                                    </td>
                                </tr>
                            ))}
                ...
                <div className="p-10 flex flex-wrap items-center justify-between border-t border-[var(--border-color)] bg-[var(--bg-color)]/20 shadow-inner">
                     <div className="flex items-center gap-4">
                        <div className="p-3 rounded-[1.5rem] bg-white border border-gray-100 shadow-xl group">
                             <ShieldCheck className="text-indigo-600 animate-spin-slow group-hover:rotate-180 transition-transform duration-1000" size={24} />
                        </div>
                        <div className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em] italic">
                             Global Acquisition Flux: <span className="text-indigo-600 not-italic font-black ml-1">{leads.length} Verified Signatures</span>
                        </div>
                     </div>
                     <div className="flex items-center gap-2">
                        <button className="px-6 py-4 bg-[var(--surface-color)] border border-[var(--border-color)] rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition50 active:scale-95 transition-all shadow-xl shadow-indigo-100/10">Synchronize System</button>
                     </div>
                </div>
            </div>
        </div>
